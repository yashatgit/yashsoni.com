...
import MediaWithCaption, { Caption } from '../elements/media';

const StyledImageContainer = ({ children }) => {
  const { isDarkMode } = useTheme();

  return (
    <div className={`my-4 image-container ${isDarkMode ? 'dark-gallery' : ''}`}>
      {children}
    </div>
  );
};
...
const CustomRenderComponent = (props) => {
  ...
  return (
    <div
      key={key}
      style={{ margin: margin, display: 'block', cursor: 'zoom-in' }}
    >
