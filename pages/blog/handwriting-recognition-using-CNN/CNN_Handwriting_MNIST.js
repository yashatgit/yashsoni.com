...
const HandwritingRecogniser = () => {
  ...
  const predictNumber = React.useCallback(
    async inputData => {
      ...
      const predictionProbs = await model.predict([tf.tensor(inputData).reshape([1, 28, 28, 1])]).dataSync();
      ...
      setPredictionSummary({ prediction: finalPrediction, prob: (maxProbability * 100).toFixed(2) });
  ...
  return (
    <>
      <FreehandDrawingCanvas size={300} ref={canvasRef} onDrawEnd={predictNumber} />
      <div>
        <button
          onClick={() => {
      ...
      <div>
        {modelState !== 'PREDICTED' ? (
          ...
          <div>
            <div>
              Predicted <strong>{predictionSummary.prediction}</strong> with <strong>{predictionSummary.prob}%</strong>{' '}
              confidence.
            </div>
